...
public class LiferayBuilder extends Builder {
    ...
    public boolean perform(AbstractBuild build, Launcher launcher, BuildListener listener) throws IOException, InterruptedException {
        ...
        String expandedWarFile = env.expand(warFile);
        String expandedWarFileDirectory = env.expand(warFileDirectory);

        if (verbose) {
            listener.getLogger().println("Selected plugin is " + pluginName);
            listener.getLogger().println("Selected war file directory is " + expandedWarFileDirectory);
            listener.getLogger().println("Selected war file is " + expandedWarFile);
        ...
        scanner.setIncludes(new String[]{expandedWarFile});
        scanner.setBasedir(expandedWarFileDirectory);
        ...
        if (files.length < 0) {
            listener.getLogger().println("NO FILE FOUND FOR '" + expandedWarFile + "' IN '" + expandedWarFileDirectory + "'");
            build.setResult(Result.FAILURE);
            return false;
        }
        if (files.length > 1) {
            listener.getLogger().println("MORE THAN ONE FILE FOUND FOR '" + expandedWarFile + "' IN '" + expandedWarFileDirectory + "'");
            build.setResult(Result.FAILURE);
            return false;
        }

        String fileInDirectory = expandedWarFileDirectory + "/" + files[0];
